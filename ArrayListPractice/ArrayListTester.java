...
public class ArrayListTester
{
    ...
    public static int countLetters(ArrayList<String>list){
        ...
        for (int i = 0; i < list.size(); i++){
            letCount += list.get(i).length();
        }
        return letCount;
    }
    public static int findPosition(int find, ArrayList<Integer>list){
    ...
    public static ArrayList<String> parseIntoArrayList(String input)
    {
        ...
        for(int i = 0; i < input.length(); i++)
        {
            if (input.substring(i, i+1).equals(" ")){
                ...
                parsedList.add(input.substring(i, i+1));
            }
        }
        return parsedList;
    }
    public static String findMin(ArrayList<Person>list){
        Person young = list.get(0);
        for(int i = 0; i < list.size(); i++)
        {
            if(list.get(i).returnAge() < young.returnAge())
            {
                young = list.get(i);
            }
        }
        return young.returnName();
    }
}
